...
def measure_metrics(metric_data, metric_groups, return_tensor=False):
    ...
    metric_group_fns = {'disp_metrics': measure_disp_metrics,
                        ...
                        'edge_metrics': measure_edge_metrics,
...
def measure_edge_metrics(metric_data):
    # unpack metric data, keys must match metric_data input
    img = metric_data['target_edges']
    img_pred = metric_data['target_edges_pred']  # (N, 1, *sizes)
    ...
    return {'rmse_edges': calculate_rmse(img, img_pred)}
...
def calculate_dice(mask1, mask2, label_class=0):
    ...
    # axes = tuple(range(2, mask1.ndim+1))
    axes = tuple(range(1, mask1.ndim))
