...
import torch
from utils.image import crop_and_pad, normalise_intensity, create_edge_map
...
def _load2d(data_path_dict):
    ...
    for name, data_path in data_path_dict.items():
        ...
        if load_nifti(data_path).shape[-1] == 1:
            data_dict[name] = load_nifti(data_path)[0].transpose(2, 0, 1)
            # data_dict[name] = load_nifti(data_path).transpose(2, 1, 0)
        else:
            data_dict[name] = load_nifti(data_path)[np.newaxis, :, :, 105]
    return data_dict
...
def _apply_mask(data_dict):
    data_dict['target'] = data_dict['target'] * data_dict['target_mask']
    data_dict['source'] = data_dict['source'] * data_dict['source_mask']
    if 'target_original' in data_dict.keys():
        data_dict['taget_original'] = data_dict['target_original'] * data_dict['source_mask']
    data_dict['target_seg'] = data_dict['target_seg'] * data_dict['target_mask']
    data_dict['source_seg'] = data_dict['source_seg'] * data_dict['source_mask']

    return data_dict

def _create_edge_map(data_dict, keys=None, N=4):
    # create edge map for the source and target images
    if keys is None:
        keys = {'target_edges', 'source_edges', 'target_original_edges'}

    for k, x in data_dict.items():
        if k in keys:
            # if k == 'source_edges':
            #     N = 10
            data_dict[k] = create_edge_map(x, N=N)
